// Load shared owner sidebar HTML and mark the active item.
(function () {
  window.handleOwnerSidebarLogout = async function handleOwnerSidebarLogout() {
    try {
      ...
      console.warn('Owner sidebar logout fallback.', error);
    }
  ...
  async function loadSidebar() {
    ...
    let html = '';
    try {
      const response = await fetch('ownerSidebar.html');
      if (!response.ok) throw new Error('Failed to fetch ownerSidebar.html');
      html = await response.text();
    } catch (error) {
      console.warn('Using fallback owner sidebar.', error);
      html = `
<nav class="w-full bg-surface border-b border-outline-variant sticky top-0 z-40">
  <div class="flex items-center justify-between h-16 px-6">
    ...
    <div class="hidden md:flex items-center gap-6">
      <a class="nav-link text-on-surface-variant hover:text-primary transition-colors pb-1" href="submission1.html">Đăng ký ký gửi</a>
      <a class="nav-link text-on-surface-variant hover:text-primary transition-colors pb-1" href="ownerSignmentTracking.html">Theo dõi ký gửi</a>
    </div>
    <div class="flex items-center gap-3">
      <button class="px-4 py-1.5 rounded-full border border-outline-variant text-secondary hover:bg-surface-container-low transition-colors" type="button" onclick="return window.handleOwnerSidebarLogout && window.handleOwnerSidebarLogout();">Đăng xuất</button>
